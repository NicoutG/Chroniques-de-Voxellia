public class Joueur {
    ...
    public boolean deplacer (Bloc [][][] terrain, BlocType [] blocs, int depx, int depy, int depz) {
        ...
        if (0<=x2 && x2<terrain.length && 0<=y2 && y2<terrain[x2].length && 0<=z2 && z2<terrain[x2][y2].length) {
            if ((z==0 || depz!=0 || (terrain[x][y][z-1]!=null && !(terrain[x][y][z-1] instanceof BlocMortel))) && (terrain[x2][y2][z2]==null || terrain[x2][y2][z2].deplacer(terrain,blocs,x2,y2,z2,depx,depy,depz,this))) {
                depX=depx;
                depY=depy;
                x=x2;
                x2=x-depx;
